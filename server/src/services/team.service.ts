...
export class TeamService {
  ...
  async getTeamMemberByMatricula(matricula: string): Promise<Team> {
    try {
      if (!matricula) {
        throw new AppError("Erro ao buscar membro de automação, matricula nao fornecida", 400)
      }

      const member = await this.teamRepository.findOne({
        where: {
          registration: matricula.toString()
        }
      })
      if (!member) {
        throw new AppError("Mebro da equipe de automação não encontradao", 404)
      }
      ...
      throw new AppError("Erro ao coletar membro da equipe de automação pela matricula", 500)
    }
  }
}
